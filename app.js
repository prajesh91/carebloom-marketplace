...
function setupMarketplace() {
  ...
  form.addEventListener('submit', async event => {
    ...
    const skill = document.getElementById('request-skill').value;
    const language = document.getElementById('request-language').value;
    const date = dateInput.value;
    const hours = parseFloat(document.getElementById('request-hours').value);
    ...
    try {
      const res = await fetch('/api/care-requests', {
        ...
        body: JSON.stringify({ recipient, city, skill, language, date, hours, notes, consentGranted })
      });
      if (!res.ok) {
      ...
      state.marketplace.requests.unshift(data.request);
      
      renderRequestMatches(data.request);
...
function scoreProviderForRequest(provider, request) {
  let score = 42;
  if (provider.vetted) score += 18;
  if (provider.city === request.city) score += 20;
  if (provider.skills.includes(request.skill)) score += 14;
  if (provider.languages.includes(request.language)) score += 5;
  score += Math.round((provider.rating - 4) * 4);
  return Math.min(99, Math.max(0, score));
}

function renderRequestMatches(request) {
  const results = document.getElementById('marketplace-results');
  const container = document.getElementById('request-provider-results');
  const summary = document.getElementById('request-match-summary');
  const confirmed = document.getElementById('confirmed-visit');
  const matches = state.providers
    .map(provider => ({ ...provider, score: scoreProviderForRequest(provider, request) }))
    .sort((a, b) => b.score - a.score);

  summary.textContent = `${matches.length} vetted professionals ranked for ${request.skill.toLowerCase()} in ${request.city}.`;
  container.innerHTML = '';
  confirmed.hidden = true;

  matches.forEach(provider => {
    const card = document.createElement('article');
    card.className = 'request-provider-card';
    const reasons = [];
    if (provider.city === request.city) reasons.push('Local to you');
    if (provider.skills.includes(request.skill)) reasons.push('Care skill match');
    if (provider.languages.includes(request.language)) reasons.push(`${request.language} speaking`);
    if (provider.vetted) reasons.push('VSS verified');
    card.innerHTML = `
      <div class="provider-card-top"><div><h4>${provider.name}</h4><p class="provider-meta">${provider.type}</p></div><span class="match-score">${provider.score}% fit</span></div>
      <p class="provider-meta">★ ${provider.rating.toFixed(1)} (${provider.reviews} reviews) · ${provider.experience} experience</p>
      <div class="fit-reasons">${reasons.map(reason => `<span>${reason}</span>`).join('')}</div>
      <div class="provider-book-row"><span>$${provider.hourlyRate}/hr</span><button class="btn-primary" type="button">Choose ${provider.name.split(' ')[0]}</button></div>`;
    card.querySelector('button').addEventListener('click', () => confirmMarketplaceBooking(provider, request));
...
async function confirmMarketplaceBooking(provider, request) {
  try {
    ...
    // Update calendar: mark today's date as active (day of month)
    ...
    // Reset & start GPS simulation
    stopGpsSimulation();
    state.gps.providerX = 40;
    state.gps.providerY = 220;
    startGpsSimulation();

    // Re-render invoice list
    renderInvoiceList();

    const confirmed = document.getElementById('confirmed-visit');
    confirmed.innerHTML = `<span class="step-badge">✓</span><div><h3>Care visit requested</h3><p>${provider.name} has been selected for ${request.recipient}'s ${request.hours}-hour visit on ${request.date}. Your demo invoice <strong>${data.invoice.id}</strong> is ready in Invoices & Billing.</p></div>`;
    confirmed.hidden = false;
    confirmed.scrollIntoView({ behavior: 'smooth', block: 'center' });
    showToast(`Visit requested with ${provider.name}. Invoice ${data.invoice.id} created.`);
...
function runAIMatching() {
  ...
  const matched = state.providers.map(p => {
  ...
  matched.forEach(p => {
    ...
    card.innerHTML = `
      <div class="match-score-badge">Match: ${p.score}%</div>
      ...
      <div class="provider-details">
        <h4>${p.name} <span class="badge badge-primary" style="font-size: 0.65rem; padding: 0.15rem 0.4rem;">VSS Approved</span></h4>
...
function startGpsSimulation() {
  ...
  state.gps.intervalId = setInterval(() => {
    // Walk provider closer to target
    const dx = state.gps.targetX - state.gps.providerX;
    const dy = state.gps.targetY - state.gps.providerY;
    ...
    if (distance < 5) {
      ...
      statusText.textContent = "Checked-in (GPS Verified) ✓";
      statusText.style.color = "var(--accent-forest)";
      stopGpsSimulation();
    } else {
      // Step towards target
      const stepX = (dx / distance) * 12;
      const stepY = (dy / distance) * 12;
      ...
      const realDistMeters = Math.round(distance * 3.5);
      distanceText.textContent = `En Route (${realDistMeters}m away)`;
      statusText.textContent = "Active Caregiver Commute";
